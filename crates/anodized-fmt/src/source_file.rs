...
use crate::{
    ...
    collect_comments::line_column_to_byte,
    config::Config,
...
fn format_source(
    ...
    for spec_attr in spec_attrs {
        ...
        // Extract the attribute source text to check for problematic comments
        ...
        // Skip formatting if there are inline comments or comments in nested structures
        // These would be lost during prettyplease/quote formatting
        if crate::collect_comments::has_problematic_comments(&attr_source) {
            continue;
        }

        // Extract standalone comments from the attribute source
        let comments =
